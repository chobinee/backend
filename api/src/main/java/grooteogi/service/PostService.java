...
public class PostService {
  ...
  public LikeDto.Response modifyHeart(Integer postId, Integer userId) {
    ...
    if (heart.isEmpty()) {
      ...
      likeResponse = LikeDto.Response.builder().liked(true).count(post.get().getHearts().size()).build();
    } else {
      heartRepository.delete(heart.get());
      likeResponse = LikeDto.Response.builder().liked(false).count(post.get().getHearts().size()).build();
    }
